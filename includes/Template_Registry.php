...
final class Template_Registry {
	...
	public static function templates(): array {
		...
		if ( ! is_array( $template_dirs ) ) {
			return apply_filters( 'wp_pwa_builder_templates', array() );
		}
	...
	private static function load_template_config( string $template_key, string $template_dir ): ?array {
		...
		return array(
			'name'    => isset( $config['name'] ) && is_string( $config['name'] ) ? $config['name'] : ucwords( str_replace( array( '-', '_' ), ' ', $template_key ) ),
			'file'    => $template_path,
			'niches'  => self::string_list( $config['niches'] ?? array( 'igaming', 'dating' ) ),
			'styles'  => self::string_list( $config['styles'] ?? array() ),
			'scripts' => self::string_list( $config['scripts'] ?? array() ),
